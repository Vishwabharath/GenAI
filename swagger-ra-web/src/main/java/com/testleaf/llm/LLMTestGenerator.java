...
public class LLMTestGenerator {
     ...
     */
    public String generateTestScripts(String apiDetails, List<String> testTypes) {
    ...
    public String generateTestCases(String apiDetails, List<String> testTypes) {
        ...
        if (testTypes == null || testTypes.isEmpty()) {
            ...
            if (types.size() == 1) {
                ...
                String joined = String.join(" and ", types);
                testTypeLine = "- Include Only " + joined + " tests\n";
            }
        }
        ...
        // Build the user prompt using the API details.
        String userPrompt = "Generate test cases based on given user story with the following specifications:\n"
                + apiDetails;
        ...
        try {
            ...
            String systemContent =
                    "You are a helpful assistant that generates test cases based on user stories. "
                            + "Your response must follow ISTQB standards and industry best practices. "
                            + "- Each test case must include: Test Case ID, Title, Description, Expected Results and Priority. "
                            + "- Ensure test cases are independent, repeatable, and maintainable. "
                            + "[IMPORTANT]"+testTypeLine
                            + "[IMPORTANT]- Generate only the mentioned test types"
                            + "- Use clear and concise language in test steps, avoiding ambiguity. "
                            + "- Use Tags to identify positive, negative and edge cases. "
                            + "- Prioritize test cases based on risk, complexity, and business impact. "
                            + "- Include both functional and non-functional test cases where necessary. "
                            + "- If the user story involves APIs, include validation for status codes, response structure, and performance. "
                            + "- If UI-based, include validation for usability, accessibility, and responsiveness. "
                            + "- Ensure compatibility with automated test execution where feasible. "
                            + "- Use lines to separate the test cases and present it in a easy to readable format";
    ...
    public String generateTestScripts(String apiDetails) {
        return generateTestScripts(apiDetails, new ArrayList<>());
    }
